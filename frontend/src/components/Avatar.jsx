...
export function Avatar({ avatar, size = 'md', className = '', border = true }) {
  ...
  return <motion.div whileHover={{ y: -2, rotate: 2 }} className={`grid place-items-center rounded-2xl bg-gradient-to-br from-[#27352e] to-[#111719] ${border ? 'border border-lime/40' : ''} ${sizes[size]} ${className}`}><span>{emoji}</span></motion.div>;
}
