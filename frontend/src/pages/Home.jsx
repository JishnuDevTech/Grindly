...
import { ArrowUpRight, Bot, CheckCircle2, ChevronDown, Coins, Flame, Plus, Search, Sparkles, Swords, Target, Trophy, Users, Zap } from 'lucide-react';
...
export default function Home({ user, quests, friends = [], activeQuestId, onStart, onComplete, onCreateQuest, onOpenAssistant }) {
  ...
  return <div className="min-h-screen bg-ink">
    <header className="border-b border-white/8 bg-ink/75 px-5 py-5 backdrop-blur-xl sm:px-8 lg:px-12 lg:py-7">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="dashboard-campaign-panel mt-5"><div className="dashboard-campaign-copy"><div className="flex items-center gap-2"><Swords size={16} className="text-lime" /><p className="eyebrow text-lime">Daily campaign · chapter one</p></div><h2 className="mt-3 font-display text-2xl font-extrabold sm:text-3xl">Make the next move count.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-300">Your run is built one focused decision at a time. Pick a quest, protect the active slot, and let the reward follow the work.</p><div className="mt-5 flex flex-wrap gap-2"><span className="dashboard-pill"><Target size={13} /> {incompleteQuests.length} open quests</span><span className="dashboard-pill"><Flame size={13} /> {user.streak} day streak</span><span className="dashboard-pill"><Trophy size={13} /> #{user.globalRank} rank</span></div></div><div className="dashboard-objective"><p className="eyebrow">Next objective</p><p className="mt-2 truncate font-display text-lg font-bold">{incompleteQuests[0]?.title || 'Create your first quest'}</p><p className="mt-1 text-xs leading-5 text-muted">{incompleteQuests[0] ? `${incompleteQuests[0].estimatedMinutes || incompleteQuests[0].duration || 30} minutes · ${incompleteQuests[0].difficulty || 'Medium'} difficulty` : 'A small, finishable quest is enough to begin.'}</p><div className="mt-4 flex items-center gap-2 text-xs font-bold text-lime"><span className="status-dot" /> Ready when you are</div></div></div>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-9">
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-panel p-5"><div className="flex items-center justify-between"><p className="eyebrow">Momentum</p><ArrowUpRight size={16} className="text-lime" /></div><div className="mt-5 flex items-end gap-2"><span className="font-display text-4xl font-extrabold">{user.completedQuests || 0}</span><span className="mb-1 text-sm text-lime">quests</span></div><p className="mt-1 text-xs text-muted">Completed quests recorded on your account</p><div className="mt-5 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-lime" style={{ width: `${Math.min(100, (user.completedQuests || 0) * 2)}%` }} /></div></div>
          <div className="rounded-2xl border border-white/10 bg-panel p-5"><div className="flex items-center gap-2"><Users size={15} className="text-[#8fd9ff]" /><p className="eyebrow">Your circle</p></div>{friends.length ? <><p className="mt-4 text-sm leading-6 text-slate-300">You are connected with <span className="font-bold text-white">{friends.length} {friends.length === 1 ? 'friend' : 'friends'}</span>.</p><p className="mt-2 text-xs text-muted">Open Ranks to view their real progress.</p></> : <p className="mt-4 text-sm leading-6 text-muted">No friends yet. Open Ranks to search for people you know.</p>}</div>
        </aside>
      </div>
    </main>
