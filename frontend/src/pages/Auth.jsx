...
export default function Auth({ onAuthenticated, onBack }) {
  ...
  const submitEmail = (event) => {
    ...
    if (mode === 'signup') sessionStorage.setItem('grindly-pending-agreements', JSON.stringify(agreements));
    run(() => mode === 'signin'
...
function AgreementList({ agreements, setAgreements }) { const items = [{ key: 'terms', label: 'I agree to the', link: 'Terms & Conditions' }, { key: 'privacy', label: 'I have read and agree to the', link: 'Privacy Policy' }, { key: 'community', label: 'I agree to the', link: 'Community Guidelines' }]; return <div className="mt-5 rounded-2xl border border-white/10 bg-white/[.03] p-4"><p className="field-label">Before you begin</p>{items.map((item) => <label key={item.key} className="mt-3 flex items-start gap-3 text-xs leading-5 text-muted"><input type="checkbox" checked={agreements[item.key]} onChange={(event) => setAgreements((current) => ({ ...current, [item.key]: event.target.checked }))} className="mt-1 accent-lime" /><span>{item.label} <strong className="font-bold text-lime">{item.link}</strong>.</span></label>)}</div>; }
