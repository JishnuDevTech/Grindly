...
function ShopItem({ item, index, user, onPurchase, onEquip, canAfford }) {
  ...
  return <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .05 }} className={`group overflow-hidden rounded-3xl border bg-panel ${style.border} ${style.glow}`}>
    <ArtifactPreview item={item} user={user} style={style} rarity={rarity} />
...
function ArtifactPreview({ item, user, style, rarity }) {
  ...
  return <div className={`relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-br ${style.panel}`}>
    <div className="artifact-stars" />
    {category === 'Profile Frame' ? <div className={`artifact-frame artifact-frame-${rarity.toLowerCase()} relative grid h-28 w-28 place-items-center ${style.glow}`}><Avatar avatar={user.avatar} size="lg" border={false} /></div> : category === 'Nameplate' ? <div className={`artifact-nameplate artifact-nameplate-${rarity.toLowerCase()} border px-5 py-3 text-center`}><div className="flex items-center justify-center gap-2"><Star size={11} /><span className="font-mono text-[.58rem] tracking-[.24em]">LEVEL {user.level}</span><Star size={11} /></div><p className="mt-1 font-display text-lg font-extrabold">@{user.username || 'grinder'}</p></div> : category === 'Avatar Effect' ? <div className={`artifact-avatar-effect artifact-avatar-effect-${rarity.toLowerCase()} relative rounded-full p-3 ${style.glow}`}><Avatar avatar={user.avatar} size="lg" /></div> : category === 'Theme' ? <div className={`artifact-theme artifact-theme-${rarity.toLowerCase()} w-48 rounded-xl border p-3 ${style.border}`}><div className="flex items-center gap-2"><Avatar avatar={user.avatar} size="xs" border={false} /><span className="text-xs font-bold text-white">@{user.username || 'grinder'}</span></div><div className="mt-3 h-2 w-20 rounded bg-white/70" /><div className="mt-2 h-2 w-32 rounded bg-white/25" /><div className="mt-4 h-10 rounded-lg border border-white/20 bg-black/25" /></div> : category === 'XP Effect' ? <div className="artifact-xp-effect text-center"><div className="flex items-center justify-center gap-2"><Zap size={18} className="text-white" /><span className="font-display text-3xl font-extrabold text-white">+120</span></div><span className="mt-1 block font-mono text-[.58rem] tracking-[.24em] text-white/70">XP GAINED</span></div> : category === 'Badge' ? <div className={`artifact-badge artifact-badge-${rarity.toLowerCase()} grid h-24 w-24 place-items-center`}><BadgeCheck size={42} strokeWidth={1.2} /></div> : category === 'Trophy Display' ? <div className={`artifact-trophy artifact-trophy-${rarity.toLowerCase()} relative flex h-28 w-24 items-end justify-center`}><Trophy size={62} strokeWidth={1.1} /><span className="absolute bottom-0 h-2 w-20 rounded-full bg-white/30" /></div> : <div className={`relative grid h-24 w-24 place-items-center rounded-[2rem] border ${style.border} ${style.artifact} ${style.glow}`}><ArtifactIcon size={42} strokeWidth={1.2} className="text-white" /></div>}
