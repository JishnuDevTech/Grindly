...
const frameMaterialColors = {
  wood: '#765239', iron: '#65717b', bronze: '#a9673b', copper: '#dd8a62',
  silver: '#d9e5ed', steel: '#90a4b8', titanium: '#b8c5d8', gold: '#f6c85f',
  'rose-gold': '#f3a5a0', platinum: '#e9f3ff', obsidian: '#596277', pearl: '#f9e9d2',
  sapphire: '#63b5ff', ruby: '#ff6f83', emerald: '#72f0b0', amethyst: '#d2a1ff',
  moonstone: '#9fe9ff', meteor: '#ff9873', crystal: '#e4fcff', diamond: '#f6ffff',
};
...
function ShopItem({ item, index, user, onPurchase, onEquip, canAfford }) {
  ...
  return <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .05 }} whileHover={{ y: -5 }} className={`group overflow-hidden border bg-panel ${style.border} ${style.glow} vault-artifact-card`}>
    ...
    <div className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-display font-bold">{item.name}</p><p className="mt-1 text-xs text-muted">{item.category} · {item.material ? `${item.material} artifact` : locked ? 'Discovery item' : 'Vault item'}</p></div><span className={`font-mono text-[.6rem] font-bold tracking-widest ${getRarityColor(rarity)}`}>{rarity.toUpperCase()}</span></div>
