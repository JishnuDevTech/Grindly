...
import { BadgeCheck, Check, Coins, Crown, Frame, Gift, Lock, Orbit, Palette, Sparkles, Shield, ShoppingBag } from 'lucide-react';
...
export default function Shop({ user, items, onPurchase }) {
  ...
  return <div className="min-h-screen bg-ink"><div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
    ...
    <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filteredItems.map((item, index) => <ShopItem key={item.id} item={item} index={index} user={user} onPurchase={onPurchase} canAfford={user.coins >= item.price} />)}</div>
...
function ShopItem({ item, index, user, onPurchase, canAfford }) {
...
function ArtifactPreview({ item, user, style, rarity }) {
  ...
  return <div className={`relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-br ${style.panel}`}>
    ...
    {category === 'Profile Frame' ? <div className={`relative grid h-28 w-28 place-items-center rounded-[2rem] border-[6px] ${style.border} ${style.glow} ${rarity === 'Mythic' ? 'animate-pulse' : ''}`}><Avatar avatar={user.avatar} size="lg" border={false} /></div> : category === 'Nameplate' ? <div className={`rounded-xl border px-5 py-3 text-center ${style.border} ${style.artifact}`}><p className="font-mono text-[.58rem] tracking-[.24em] text-white/70">LEVEL {user.level}</p><p className="mt-1 font-display text-lg font-extrabold text-white">@{user.username || 'grinder'}</p></div> : category === 'Avatar Effect' ? <div className={`relative rounded-full p-2 ${style.artifact} ${style.glow}`}><Avatar avatar={user.avatar} size="lg" /></div> : category === 'Theme' ? <div className={`w-44 rounded-xl border p-3 ${style.border} ${style.artifact}`}><div className="flex items-center gap-2"><Avatar avatar={user.avatar} size="xs" border={false} /><span className="text-xs font-bold text-white">@{user.username || 'grinder'}</span></div><div className="mt-3 h-2 w-20 rounded bg-white/50" /><div className="mt-2 h-2 w-32 rounded bg-white/20" /><div className="mt-4 h-10 rounded-lg border border-white/15 bg-black/20" /></div> : <div className={`relative grid h-24 w-24 place-items-center rounded-[2rem] border ${style.border} ${style.artifact} ${style.glow}`}><ArtifactIcon size={42} strokeWidth={1.2} className="text-white" />{category === 'XP Effect' && <span className="absolute -right-7 -top-2 font-display text-sm font-extrabold text-white">+XP</span>}</div>}
