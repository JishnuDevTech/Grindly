...
const preferenceGroups = [
  ...
  { title: 'Notifications', rows: [{ id: 'quest_reminders', label: 'Quest reminders', detail: 'A nudge when a focused session is waiting', icon: Bell }, { id: 'reward_notifications', label: 'Reward notifications', detail: 'Level-ups and new unlocks', icon: Bell }, { id: 'friend_activity', label: 'Friend activity', detail: 'Milestones from your circle', icon: UserRound }] },
...
export default function SettingsPage({ user, theme, onThemeChange, preferences = {}, onPreferencesChange, onAvatarChange, onSignOut }) {
  ...
  const docs = {
    account: ['Account settings', 'Update your identity and sign-in details.', ['Your Grindly account is secured through Firebase sign-in.', 'Use the Profile and onboarding controls to update your public identity.', 'Sign out ends this browser session immediately.']],
    terms: ['Terms & Conditions', 'The rules of the journey.', ['Use Grindly honestly and do not manipulate timers, rankings, or rewards.', 'Cosmetics and rewards are gameplay data tied to your account.', 'We may restrict abusive or automated activity.']],
    privacy: ['Privacy Policy', 'How Grindly handles your data.', ['Grindly stores progression, quests, preferences, and social relationships to provide the game.', 'Visibility settings control what other players can see.', 'You can request account help through the project owner.']],
    community: ['Community Guidelines', 'Keep the grind respectful.', ['Respect other players. No harassment, hate, spam, or inappropriate content.', 'Do not exploit reward systems or attempt to manipulate rankings.', 'Report behavior that makes the community unsafe.']],
  };
...
function DocumentPage({ document, onBack, onSignOut }) { const [title, detail, paragraphs] = document; return <div className="min-h-screen bg-ink"><div className="mx-auto max-w-2xl px-5 py-10 sm:px-8"><button onClick={onBack} className="btn-secondary">Back to settings</button><p className="eyebrow mt-10 text-lime">Grindly document</p><h1 className="mt-2 font-display text-4xl font-extrabold">{title}</h1><p className="mt-3 text-muted">{detail}</p><article className="card mt-8 space-y-5 text-sm leading-7 text-slate-300">{paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</article>{title === 'Account settings' && <button onClick={onSignOut} className="mt-6 btn-secondary">Sign out</button>}</div></div>; }
