...
export default function Leaderboard({ user, entries: allEntries }) {
  const [tab, setTab] = useState('global');
  const entries = tab === 'global' ? allEntries : allEntries.filter((player) => player.rank <= 5 || player.isCurrentUser);
  return (
    <div className="min-h-screen bg-ink">
      <div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">{entries.slice(0, 3).map((player, index) => <PodiumCard key={player.username} player={player} place={index + 1} />)}</div>
...
function PodiumCard({ player, place }) {
  ...
  const medals = { 1: '🥇', 2: '🥈', 3: '🥉' };
  return <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: place * .08 }} className={`relative overflow-hidden rounded-3xl border bg-gradient-to-br p-5 ${colors[place]}`}><div className="absolute right-4 top-3 text-3xl opacity-70">{medals[place]}</div><div className="flex items-center gap-3"><span className="font-mono text-xs text-muted">0{place}</span><Avatar avatar={player.avatar} size="md" border={false} /><div className="min-w-0"><p className="truncate font-bold">{player.displayName}</p><p className="truncate text-xs text-muted">@{player.username}</p></div></div><div className="mt-6 flex items-end justify-between"><div><p className="eyebrow">Level</p><p className="mt-1 font-display text-2xl font-extrabold">{player.level}</p></div><div className="text-right"><p className="eyebrow">XP points</p><p className="mt-1 font-mono text-lg font-bold text-[#ffd27a]">{player.xp.toLocaleString()}</p></div></div></motion.div>;
}
