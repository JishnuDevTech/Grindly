...
export default function Leaderboard({
  ...
  const changeRequest = async (request, action) => {
    try {
      ...
      updateRequests({ ...requests, incoming: requests.incoming.filter((item) => item.id !== request.id) });
    } catch (e) { setError(e.message); }
  };
  ...
  return (
    <div className="min-h-screen bg-ink"><div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
      ...
      {entries.length ? <div className="mt-8 grid gap-4 md:grid-cols-3">{entries.slice(0, 3).map((player, index) => <PodiumCard key={player.username} player={player} place={index + 1} onOpen={openProfile} />)}</div> : <div className="mt-8 rounded-3xl border border-dashed border-white/15 bg-panel p-8 text-center text-sm text-muted">No friends are ranked yet. Search for a grinder and send a request to build your circle.</div>}
      <div className="mt-8 flex items-center justify-between"><div><p className="eyebrow">{tab === 'global' ? 'All players' : 'Your circle'}</p><h2 className="mt-2 font-display text-2xl font-extrabold">This week</h2></div><button onClick={() => setSocialOpen((open) => !open)} className="icon-button" aria-label="Search players"><Search size={17} /></button></div>
      {socialOpen && <SocialPanel query={query} setQuery={setQuery} search={search} results={results} sendRequest={sendRequest} friends={friends} requests={requests} changeRequest={changeRequest} openProfile={openProfile} />}
      ...
      {profile && <ProfileModal profile={profile} loading={profileLoading} onClose={() => setProfile(null)} onSendRequest={sendRequest} onChangeRequest={changeRequest} onRemoveFriend={removeFriend} />}
