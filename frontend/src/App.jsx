  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const [me, nextQuests, nextLeaderboard, nextShop, nextFriends, nextNotifications, nextPreferences, nextInventory] = await Promise.all([api.getMe(token), api.getQuests(token), api.getLeaderboard(token), api.getShop(token), api.getFriendLeaderboard(token), api.getNotifications(token), api.getPreferences(token), api.getInventory(token)]);
        if (!cancelled) {
          setUser(me);
          setQuests(nextQuests);
          setLeaderboard(nextLeaderboard);
          setFriendLeaderboard(nextFriends);
...
function CompanionPanel({ onClose, user, quests }) { const [message, setMessage] = useState(''); const activeQuest = quests.find((quest) => quest.status === 'active'); const [messages, setMessages] = useState([{ id: 1, from: 'bot', text: activeQuest ? `I’m watching ${activeQuest.title}. Stay with it and I’ll help you finish strong.` : `Welcome back, ${user.displayName || 'grinder'}. What are we turning into a quest today?` }]); const respond = (text) => { const normalized = text.toLowerCase(); if (normalized.includes('reward') || normalized.includes('xp')) return `Rewards are earned from completed focus time, not clicks. You’re level ${user.level} with ${user.experience} XP, so the next win moves your character forward.`; if (normalized.includes('task') || normalized.includes('quest')) return activeQuest ? `Your active quest is ${activeQuest.title}. Finish that timer before starting another quest.` : 'Tell me the task, its realistic minutes, and how difficult it feels. I’ll turn it into a clear quest brief.'; if (normalized.includes('streak')) return `Your current streak is ${user.streak} days. Protect it with one honest, finishable quest today.`; return 'I’m your Grindly focus co-pilot. I can help shape a task, explain fair rewards, or keep you accountable to the quest already running.'; }; const send = (value = message) => { const text = value.trim(); if (!text) return; setMessages((current) => [...current, { id: Date.now(), from: 'user', text }, { id: Date.now() + 1, from: 'bot', text: respond(text) }]); setMessage(''); }; return <motion.div className="fixed inset-0 z-50 flex items-end justify-end bg-black/50 p-4 backdrop-blur-sm sm:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="flex h-[min(680px,calc(100vh-2rem))] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-white/15 bg-panel shadow-2xl"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div className="flex items-center gap-3"><GrindlyCharacter mood="talking" size="sm" /><div><p className="font-semibold">Grindly</p><p className="text-xs text-muted">Your momentum co-pilot</p></div></div><button onClick={onClose} className="icon-button"><X size={18} /></button></div><div className="flex-1 space-y-4 overflow-y-auto p-5">{messages.map((entry) => <div key={entry.id} className={`flex ${entry.from === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${entry.from === 'user' ? 'rounded-br-sm bg-lime text-ink' : 'rounded-bl-sm bg-white/8 text-slate-200'}`}>{entry.text}</div></div>)}<div className="flex flex-wrap gap-2"><button onClick={() => send('How do rewards work?')} className="chip-button">Explain rewards</button><button onClick={() => send('Help me make a task')} className="chip-button"><Plus size={13} /> Make a task</button><button onClick={() => send('How is my streak?')} className="chip-button">Check streak</button></div></div><form onSubmit={(event) => { event.preventDefault(); send(); }} className="border-t border-white/10 p-4"><div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 p-2"><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Tell me what you need to do..." className="min-w-0 flex-1 bg-transparent px-2 text-sm text-white outline-none placeholder:text-muted" /><button className="rounded-xl bg-lime p-2.5 text-ink" aria-label="Send message"><MessageCircle size={16} /></button></div></form></motion.div></motion.div>; }
